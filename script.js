...
paginationData.forEach((el) => {
  ...
  btnPageNumber.classList.add("page-number");

  if (el.id === "1") {
    btnPageNumber.innerHTML = `First`;
  } else if (el.id === String(paginationData.length)) {
    btnPageNumber.innerHTML = `Last`;
  } else {
    btnPageNumber.innerHTML = `${el.id}`;
  }
  if (el.id === "1") {
    nextBtn.insertAdjacentElement("afterend", btnPageNumber);
  } else {
    prevBtn.insertAdjacentElement("beforebegin", btnPageNumber);
  }
});

const pageNumber = document.querySelectorAll(".page-number");

const displayDetails = function (val) {
  ...
  if (val === "First") {
    nameDetail.innerHTML = paginationData[0].name;
    emailDetail.innerHTML = paginationData[0].email;
    description.innerHTML = `Change:${paginationData[0].id}`;
  }

  if (val === "Next") {
    const nameCheck = nameDetail.textContent;
    const newVal = paginationData.filter((el) => el.name === nameCheck)[0].id;
    nameDetail.innerHTML = paginationData[Number(newVal)].name;
    emailDetail.innerHTML = paginationData[Number(newVal)].email;
    description.innerHTML = `Change:${paginationData[Number(newVal)].id}`;
  }

  if (val === "Last") {
    nameDetail.innerHTML = paginationData[paginationData.length - 1].name;
    emailDetail.innerHTML = paginationData[paginationData.length - 1].email;
    description.innerHTML = `Change:${
  ...
  if (val === "Previous") {
    ...
    const newVal = paginationData.filter((el) => el.name === nameCheck)[0].id;
    console.log(newVal);
    nameDetail.innerHTML = paginationData[Number(newVal) - 2].name;
    emailDetail.innerHTML = paginationData[Number(newVal) - 2].email;
    description.innerHTML = `Change:${paginationData[Number(newVal) - 2].id}`;
  } else {
    nameDetail.innerHTML = paginationData[Number(val) - 1].name;
    emailDetail.innerHTML = paginationData[Number(val) - 1].email;
    description.innerHTML = `Change:${paginationData[Number(val) - 1].id}`;
  }
};

pageNumber.forEach((el) => {
  el.addEventListener("click", function () {
    displayDetails(el.textContent);
